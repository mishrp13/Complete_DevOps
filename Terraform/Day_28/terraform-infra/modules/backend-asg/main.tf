...
resource "aws_launch_template" "backend" {
  ...
  user_data = base64decode(templatefile("${path.root}/../../scripts/backend_user_data.sh",{
...
resource "aws_cloudwatch_metric_alarm" "backend_high_cpu" {
  ...
  statistic = "Average"
  threshold = 80
  alarm_description = "This backend monitors backend ec2 cpu utilization"
  alarm_actions = var.alarm_actions

  dimensions = {
    autoscaling_group_name= name_autoscaling_group.backend.name
  }
  tags=var.tags
}