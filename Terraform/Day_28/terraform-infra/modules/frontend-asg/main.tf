...
resource "aws_launch_template" "frontend" {
  ...
  user_data = base64decode(templatefile("${path.root}/../../scripts/frontend_user_data.sh", {
...
resource "aws_cloudWatch_metrice_alarm" "frontend_high_cpu" {
  ...
  stastics= "Average"
  threshold= 80
  alarm_description= "The metric monitors frontned EC2 CPU Utilization"
...
resource "aws_cloudwatch_metric_alarm" "frontend_unhealthy" {
  ...
  alarm_actions = var.alarm_sctions
